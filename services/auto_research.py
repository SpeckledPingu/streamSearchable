...
def call_llm(prompt, system_prompt="", max_tokens=2000, temperature=0, top_k=50):
    response = requests.post(
        url=os.getenv('OPENROUTER_URL'),
        headers={
            "Authorization": f"{os.getenv('OPENROUTER_API')}",
            "HTTP-Referer": f"",
            "X-Title": f"",
        },
        data=json.dumps({
...
def generate_internet_queries_prompt(task, objective):
...
def generate_internet_queries(task, objective):
    prompt = generate_internet_queries_prompt(task, objective)
    queries = call_llm(prompt)
    parsed_queries = extract_queries(queries)
    return parsed_queries, queries

def get_internet_search_articles(query, max_articles=3, max_results=10):
    with DDGS() as ddgs:
...
def generate_summary_prompt(task_details, article_title, article_content):
    """
...
def get_article_summary(task, article):
    prompt = generate_summary_prompt(task,
                                     article_title=article.title,
                                     article_content=article.text)
    summary = call_llm(prompt)
    return summary

def get_task_article_summaries(task, articles):
    article_summaries = list()
    for article in tqdm(articles):
        summary = get_article_summary(task, article)
        article_summaries.append(summary)
    return article_summaries


def create_detailed_analytical_prompt(objective, task, articles):
    """
...
def final_task_analysis(objective, task, articles):
    prompt = create_detailed_analytical_prompt(objective, task, articles)
    analysis = call_llm(prompt)
    return analysis


def clean_up_summaries(text):
    text = '\n\n'.join(text)
...
Articles:
{text}"""
    simplified_summary = call_llm(prompt)
    return simplified_summary

def research_question(objective):
    ...
    for task_id, task in tqdm(tasks.items()):
        ...
        task_data = dict()
        first_research_task = generate_internet_queries(task, objective)
        time.sleep(7)
        ...
        for query_id, query in tqdm(enumerate(internet_queries)):
            ...
            articles = get_internet_search_articles(query)

            query_data['article_objects'] = articles[0]
            query_data['article_metadata'] = articles[1]

            print("Articles:")
            print(articles[1])

            sub_summary = final_task_analysis(objective, task, articles[0])
            query_data['sub_summary'] = sub_summary
            sub_summaries.append(sub_summary)

            task_data['queries'][query_id] = query_data
            article_objs.extend(articles[0])
            time.sleep(6)
        final_summary = final_task_analysis(objective, task, article_objs)
        task_data['final_summary'] = final_summary

        time.sleep(5)
        summarized_summary = clean_up_summaries(sub_summaries)
    ...
    print(time.time() - t0)
    create_markdown_report(tracking, objective, objective)
    return tracking
...
def create_markdown_report(tracking, objective, file_name):
    ...
    sanitized_text = sanitize_markdown(markdown_report)
