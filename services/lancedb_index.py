...
class LanceDBDocument():
    def __init__(self, document:dict, title:str, text:str, fields, tags=None, date=None, file_path=None):
        self.title = document[title] if title is not None else None
...
class LanceDBIngest():
    ...
    def initialize(self):
        ...
        if self.overwrite:
            self.db.create_table(self.index_name, data=self.documents, mode='overwrite', schema=self.schema.to_arrow_schema())
    ...
    def insert(self, document):
        document['uuid'] = uuid4()
        document['vector'] = self.encoder.encode([document['text']])[0]
...
class IndexDocuments():
    ...
    def ingest(self, overwrite=False):
        ...
        lance_folder.mkdir(parents=True)
